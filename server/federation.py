...
def agg_params(params, edge_weights):
    ...
    for l in rlayers:
        ...
        for s in rsamples:
            ...
            new_sample = [np.average(i, weights = edge_weights) for i in list(zip(*temp_list))]
...
def local_updates(net_lst, data_lst, n_local_updates, testing_data, rs):
    ...
    for i in range(len(net_lst)): 
        accuracy_current_round = net_lst[i].SGD(data_lst[i], n_local_updates, batchsize, eta, testing_data, rs)

...
def init_dataset(rs, filename="diabetes.csv"):
    ...
    training_data = [(np.reshape(x,(len(x),1)),np.reshape(np.array(y),(1,1))) for x,y in  zip(np.array(X_train),y_train)]
    testing_data = [(np.reshape(x,(len(x),1)),np.reshape(np.array(y),(1,1))) for x,y in  zip(np.array(X_test),y_test)]
...
def analyze_results(results):
    ...
    for e in range(n_edges):
        for t in range(n_periods):
            mean_accuracy[e].append(*[np.mean([results[s][e][t] for s in range(n_simulations)])])
